  ...
  return new Promise<ZipScanResult>((resolve) => {
    yauzl.open(file, { lazyEntries: true, autoClose: true }, (err, zf) => {
      ...
      zf.on('end', () => {
        ...
        // META-INF/MANIFEST.MF is only required for actual JAR mods.
        // Resource packs, shader packs and data packs are plain ZIPs and
        // never have a manifest — we check this in validateJarFile based
        // on the file extension, not here.
        resolve({ ok: true, entries, hasManifest })
      })
    ...
    if (req.mods.length > 0) {
      ...
      const processMod = async (mod: ModSpec): Promise<void> => {
        ...
        try {
          ...
          // Determine the correct file extension from the resolved filename or URL.
          // Shaders and resource packs are .zip, mods are .jar.
          const resolvedExt = (() => {
            const fname = resolved.filename || resolved.downloadUrl.split('?')[0].split('/').pop() || ''
            if (fname.toLowerCase().endsWith('.zip')) return '.zip'
            return '.jar'
          })()

          const cachePath = resolved.sha1 != null
            ? this.instances.cachedModPath(resolved.sha1, resolvedExt)
          ...
          // JAR structure check only on .jar cache hits without sha1
          ...
          // For .zip files (shaders, resource packs): just check magic bytes
          if (!needsDownload && !resolved.sha1 && cachePath.toLowerCase().endsWith('.zip')) {
            try {
              ...
              fs.closeSync(fd)
              const isZip = buf[0] === 0x50 && buf[1] === 0x4b
              if (!isZip) { try { fs.unlinkSync(cachePath) } catch { /* ignore */ }; needsDownload = true }
            } catch { needsDownload = true }
          }
      ...
      // ── Preserve manually-added files in all three folders ─────────────────
      // Before wiping, snapshot any files placed there manually (NOT by the
      // launcher). Launcher-managed files always follow the pattern:
      //   <name>-<sha1hex>.(jar|zip)  (contains a hex segment at the end)
      // Any file that doesn't match is treated as user-added and preserved.
      const LAUNCHER_FILE_RE = /^.+-[0-9a-f]{8,40}(\.(jar|zip))(\.disabled)?$/i

      const snapshotExternal = (dir: string, exts: string[]): Array<{ name: string; data: Buffer }> => {
        const saved: Array<{ name: string; data: Buffer }> = []
        try {
          for (const f of fs.readdirSync(dir)) {
            if (exts.some(e => f.endsWith(e)) && !LAUNCHER_FILE_RE.test(f)) {
              try {
                saved.push({ name: f, data: fs.readFileSync(path.join(dir, f)) })
                this.emit('stderr', `[mods] preservando arquivo externo em ${path.basename(dir)}/: ${f}\n`)
              } catch { /* ignore */ }
            }
          }
      ...
      const externalMods         = snapshotExternal(modsTarget,          ['.jar', '.zip'])
      const externalResourcepacks = snapshotExternal(resourcepacksTarget, ['.zip', '.jar'])
      const externalShaderpacks   = snapshotExternal(shaderpacksTarget,   ['.zip', '.jar'])
      ...
      wipeArchives(modsTarget,          ['.jar', '.zip', '.jar.disabled'])
      ...
      for (const { cachePath, mod } of downloaded) {
        ...
        switch (kind) {
          ...
          case 'datapack':
            target = resourcepacksTarget; extension = '.zip'; break
          default:
            // If the cached file is already a .zip but wasn't detected as a
            // known pack type, still place it in mods/ but keep .zip extension
            // so Minecraft can load it (some mods ship as .zip on CurseForge)
            target = modsTarget
            extension = cachePath.toLowerCase().endsWith('.zip') ? '.zip' : '.jar'
        }
      ...
      // Restore manually-added files in all three folders
      const restoreExternal = (dir: string, files: Array<{ name: string; data: Buffer }>) => {
        for (const ext of files) {
          try { fs.writeFileSync(path.join(dir, ext.name), ext.data) } catch { /* ignore */ }
        }
      }
      restoreExternal(modsTarget,          externalMods)
      restoreExternal(resourcepacksTarget, externalResourcepacks)
      restoreExternal(shaderpacksTarget,   externalShaderpacks)

      const extTotal = externalMods.length + externalResourcepacks.length + externalShaderpacks.length
      const extMsg = extTotal > 0 ? ` + ${extTotal} arquivo(s) externos preservados` : ''
      this.progress({
        phase: 'mods',
